...
def run_crawlers(Graph: MyGraph, total_budget=100, crawlers=None, layout_pos=None, gif=False, step=1):
    if crawlers is None:
        crawlers = ['DFS']
...
def test_crawlers_drawing(graph: MyGraph):
    layout_pos = None
    ...
    ####Graph = MyGraph.new_snap(graph.snap, name='test', directed=False)
    # graph, layout_pos = test_carpet_graph(10, 8)
    # Graph = MyGraph.new_snap(graph.snap, name='test', directed=False)
...
if __name__ == '__main__':
    ...
    test_crawlers_drawing(g)  # FIXME not working?
