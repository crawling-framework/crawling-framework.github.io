...
from crawlers.basic import Crawler, PreferentialObservedDegreeCrawler, DepthFirstSearchCrawler, \
        ...
        if self.draw_mod == 'metric':
            ...
            for crawler in crawlers:
                if os.path.exists(pngs_path):
                    for file in glob.glob(pngs_path + crawler.crawler_name + "*.png"):
                        os.remove(file)
                else:
    ...
    def run(self):
                ...
                for m, metric in enumerate(self.metrics):
                    ...
                    if self.draw_mod == 'metric':
                        plt.plot(step_seq, metric_seq, marker='.',
                                 ...
                                 label=r'%s, %s' % (crawler.crawler_name, metric.name))
                        ...
                        plt.title(crawler.crawler_name + " " + str(len(crawler.crawled_set)))
                        ...
                        plt.savefig(pngs_path + 'gif{}_{}.png'.format(crawler.crawler_name, str(i).zfill(3)))
                        plt.cla()
                        # plt.show()
        ...
        if self.draw_mod == 'metric':
            ...
            for crawler in self.crawlers:
                make_gif(crawler_name=crawler.crawler_name, pngs_path=pngs_path)
                print('compiled +')
