...
def declaration_to_filename(declaration) -> str:
    ...
    def check_len(string):
        if len(string) >= 120:
            hashed = hashlib.md5(string.encode('utf-8')).digest().hex()
            print(f"Hashing too long value '{string}' -> {hashed}")
            string = hashed
        return string
    ...
    def to_filename(obj):
        ...
        if isinstance(obj, list):
            return check_len(str(list(to_filename(o) for o in obj)))
        if isinstance(obj, tuple):
            return check_len(str(tuple(to_filename(o) for o in obj)))
        if isinstance(obj, set):
            return check_len(str(set(to_filename(o) for o in obj)))
        if isinstance(obj, dict):
            return check_len(str({to_filename(ok): to_filename(ok) for ok, ov in obj.items()}))
